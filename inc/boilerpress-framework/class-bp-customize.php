...
class BP_Customize {
     ...
     *
     */
    public function get_option( $config_id = '', $field_id = '' ) {
        ...
        $default = '';
        if ( isset( $this->fields[ $field_id ] ) && isset( $this->fields[ $field_id ][ 'default' ] ) ) {
            $default = $this->fields[ $field_id ][ 'default' ];
        }
        // Make sure the config is defined
        if ( isset( $this->config[ $config_id ] ) ) {
            if ( 'option' == $this->config[ $config_id ][ 'option_type' ] ) {
                // check if we're using serialized options
                if ( isset( $this->config[ $config_id ][ 'option_name' ] ) && ! empty( $this->config[ $config_id ][ 'option_name' ] ) ) {
                    // Get all our options
                    $all_options = get_option( $this->config[ $config_id ][ 'option_name' ], array() );
    ...
    public function add_config( $config_id, $args = array() ) {
        ...
        if ( ! isset( $this->config[ $config_id ][ 'option_type' ] ) ) {
            $this->config[ $config_id ][ 'option_type' ] = 'theme_mod';
        }
    }
     ...
     */
    public function add_field( $config_id, $args ) {
        // if Kirki exists, use it.
        if ( class_exists( 'Kirki' ) ) {
            ...
            if ( 'background' == $args[ 'type' ] && isset( $args[ 'output' ] ) ) {
                if ( isset( $args[ 'default' ] ) && is_array( $args[ 'default' ] ) ) {
                    foreach ( $args[ 'default' ] as $default_property => $default_value ) {
                        ...
                        $this->fields[ $property_setting ] = $subfield;
                    }
                }
            }
                    ...
                    if ( 'typography' == $field[ 'type' ] ) {
                        foreach ( $value as $key => $subvalue ) {
                            ...
                            if ( 'variant' == $key ) {
                                ...
                                $font_weight =
                                    ( in_array( $font_weight, array( '', 'regular' ) ) ) ? '400' : $font_weight;
                                // Is this italic?
                                $is_italic                                           =
                                    ( FALSE !== strpos( $subvalue, 'italic' ) );
