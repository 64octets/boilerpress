 ...
 * Accounts for the BoilerPress Designer's content background option.
 *
 * @since 0.0.1
 * @return string the background color
 */
function boilerpress_get_content_background_color() {
	// Set the bg color var based on whether the BoilerPress designer has set a content bg color or not.
	...
	// But if the BoilerPress Designer extension is active, and the content frame option is enabled we need that bg color instead.
	if ( $content_bg_color && 'true' == $content_frame && class_exists( 'BoilerPress_Designer' ) ) {
...
/**
 * Apply inline style to the BoilerPress header.
 *
 * @uses  get_header_image()
 * @since 0.0.1
 */
function boilerpress_header_styles() {
	...
	$styles = apply_filters( 'boilerpress_header_styles', array(
 ...
 * @since 0.0.1
 */
function boilerpress_sanitize_choices( $input, $setting ) {
 ...
 * @return void
 */
function boilerpress_html_tag_schema() {
	_deprecated_function( 'boilerpress_html_tag_schema', '2.0.2' );
 ...
 * @since 1.0.3
 */
function boilerpress_sanitize_layout( $input ) {
	_deprecated_function( 'boilerpress_sanitize_layout', '2.0', 'boilerpress_sanitize_choices' );
...
/**
 * BoilerPress Sanitize Hex Color
 *
 * @param string $color The color as a hex.
 * @todo remove in 2.1.
 */
function boilerpress_sanitize_hex_color( $color ) {
	_deprecated_function( 'boilerpress_sanitize_hex_color', '2.0', 'sanitize_hex_color' );
...
function boilerpress_categorized_blog() {
	_deprecated_function( 'boilerpress_categorized_blog', '2.0' );
